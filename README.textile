...
* Provides a simple workflow engine with a sane and uniform interface for running "Pig":http://pig.apache.org, "Wukong":http://github.com/infochimps/wukong, and "R":http://www.r-project.org/ scripts
* Scripts can run themselves idempotently
* Scripts are always treated as eruby templates so variable substitution is easy
...
* A workflow has an id
* A workflow manages intermediate outputs
...
* A workflow can be run idempotently (by virtue of it being constructed from @script@ objects)
* A workflow can describe its dependency graph
* A workflow should be able to clean up its intermediate outputs
